...
def calc_xy(address_layer):
    with arcpy.da.UpdateCursor(address_layer, ["SHAPE@", "X", "Y"]) as cursor:
        for row in cursor:
            geometry = row[0]
            if geometry:
                row[1] = geometry.centroid.X
                row[2] = geometry.centroid.Y
                cursor.updateRow(row)
...
def get_amanda_cursor(server, database):
    ...
    import pyodbc
    try:
        conn_amanda = pyodbc.connect(r'DRIVER={ODBC Driver 17 for SQL Server;'
                                     f'SERVER={server};'
                                     f'DATABASE={database};'
...
def execute_amanda_sproc(self, server, database, address_layer):
    ...
    :return: this will be an integer representing the RSN value generated by the AMANDA stored procedure for each
            address record
    '''
    import pyodbc
    ...
    conn_str = (
        ...
        "Trusted_Connection=yes;AUTOCOMMIT=YES;"
        "MARS_Connection=Yes;"
    )
    amanda_db = pyodbc.connect(conn_str)
    db_cur = amanda_db.cursor()
    amanda_dict_fields = self.amanda_dict
    address_field_list = list(amanda_dict_fields.values())
        ...
        for row in cursor:
            try:
                params = [
                    ...
                    row[address_field_list.index("Street_Name")],
                    row[address_field_list.index("Street_Type")],
                    row[address_field_list.index("Direction")],
                    row[address_field_list.index("Unit_Type")],
                    row[address_field_list.index("Unit_Number")],
                    ...
                    row[address_field_list.index("Subdivision")],
                    ...
                    row[address_field_list.index("Y")],
                ...
                db_cur.execute(call_sql, params)

                # Step through until we find the result set
                while db_cur.description is None:
                    if not db_cur.nextset():
                        break

                # Fetch RSN value before commit
                row_result = db_cur.fetchone()

                amanda_db.commit()

                out_rsn = row_result[0] if row_result else -9999

                if out_rsn == -1:
                    arcpy.AddWarning(f"Row UID {row[0]}: already exists, skipped.")
                elif out_rsn == -9999:
                    arcpy.AddError(f"Row UID {row[0]}: failed to retrieve RSN.")
                else:
                    arcpy.AddMessage(f"Row UID {row[0]}: inserted RSN={out_rsn}")
                    row[address_field_list.index("RSN")] = out_rsn
                    cursor.updateRow(row)

            except Exception as e:
                arcpy.AddError(f"Row UID {row[0]}: Error - {e}")
